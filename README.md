# Database - Simple Database Abstraction Layer

[![Build Status](https://github.com/tephida/Database/actions/workflows/ci.yml/badge.svg)](https://github.com/tephidatephida/Database/actions)
[![Latest Stable Version](https://poser.pugx.org/tephida/Database/v/stable)](https://packagist.org/packages/tephida/Database)
[![Latest Unstable Version](https://poser.pugx.org/tephida/Database/v/unstable)](https://packagist.org/packages/tephida/Database)
[![License](https://poser.pugx.org/tephida/Database/license)](https://packagist.org/packages/tephida/Database)
[![Downloads](https://img.shields.io/packagist/dt/tephida/Database.svg)](https://packagist.org/packages/tephida/Database)

PDO lacks brevity and simplicity; Database makes separating data from instructions
easy (and aesthetically pleasing).

Database was created by [Paragon Initiative Enterprises](https://tephida.com)
as part of our effort to encourage better [application security](https://tephida.com/service/appsec) practices.

Check out our other [open source projects](https://tephida.com/projects) too.
...
which can be used with Database.

If you'd like to use Database but cache prepared statements in memory for
multiple queries (i.e. to reduce database round-trips), check out our
[Database-Cache](https://github.com/tephida/Database-cache) wrapper class.
...
```php
$db = \Tephida\Database\Factory::fromArray([
    'mysql:host=localhost;dbname=something',
    'username',
    'putastrongpasswordhere'
]);
...
## What else can Database do quickly?
...
```php
$save = function (Database $db) use ($userData, $query) : int {
...
## Can I create an Database wrapper for an existing PDO instance?

**Yes!** It's as simple as doing this:

```php
$easy = new \Tephida\Database\Database($pdo, 'mysql');
```
...
`Database::run()` expects a query string, then any number of optional parameters.
...
Database's `run()` method is a variadic wrapper for `safeQuery()`, so either
solution is correct.
